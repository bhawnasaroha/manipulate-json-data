...
var pokemonInfo = function(pokemonName){
  for(var x in pokemonData.pokemon){
    if(pokemonName === pokemonData.pokemon[x].name){
      document.getElementById("pokemon").innerHTML = JSON.stringify(pokemonData.pokemon[x],undefined, 2);
      document.getElementsByTagName("img")[0].setAttribute("src", pokemonData.pokemon[x].img);
    } else if(pokemonName !== pokemonData.pokemon[x].name){
       document.getElementById("notFound").innerHTML = "Sorry, no such match was found! Please, try again with another name.";
       document.getElementsByTagName("img")[0].setAttribute("alt", "No image!");
     }
  }
};
...
var normalCaseName = prompt("Enter the name of pokemon:");
console.log(capitalizeName(normalCaseName));
pokemonInfo(capitalizeName(normalCaseName));
